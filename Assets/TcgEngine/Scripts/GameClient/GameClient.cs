...
namespace TcgEngine.Client
{
    ...
    public class GameClient : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            if (game_data == null || game_data.state == GameState.Connecting)
            {
        ...
        public virtual async void ConnectToServer()
        {
            ...
            {
                TcgNetwork.Get().StartHostOffline();    //WebGL dont support hosting a game, must join a dedicated server
            }
            else if (game_settings.IsHost())
            {
                TcgNetwork.Get().StartHost(NetworkData.Get().port);       //Host a game, either solo or for P2P
            }
            else
            {
